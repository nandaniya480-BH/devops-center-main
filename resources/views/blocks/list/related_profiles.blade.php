@if(count($related) != 0)
    ...
    <div class="col-12 pb-5 pt-5">
        <h3 class="display-3 text-center mb-5">{{ __('profile_lists')['related_profile_title'] }}</h3>
        <div class="row">
                                ...
                                @if($item->item_skill_tags)
                                    <div class="d-block mt-1">
                                        @foreach($item->item_skill_tags as $tags_item)
                                            <a href="/profile/topics/{{ $tags_item }}"
                                                class="badge badge-pill badge-default mt-1 mr-1">{{ $tags_item }}</a>
                                        @endforeach
                                    </div>
                                @endif
                            </div>
